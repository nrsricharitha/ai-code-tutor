...
# Google Cloud TTS language/voice map
GCP_TTS_VOICES = {
    "english":  {"languageCode": "en-IN", "name": "en-IN-Wavenet-A"},
...
@login_required
def tts():
    """Proxy Google Cloud TTS so the API key stays server-side."""
    data = request.get_json(silent=True) or {}
    ...
    # If no GCP key configured → tell the browser to use Web Speech API fallback
    if not gcp_key:
        return jsonify({"fallback": True, "language": language})

    voice_cfg = GCP_TTS_VOICES.get(language, GCP_TTS_VOICES["english"])
    payload = {
        "input": {"text": text},
        "voice": {"languageCode": voice_cfg["languageCode"], "name": voice_cfg["name"]},
    ...
    try:
        ...
        return jsonify({"audioContent": result.get("audioContent", ""), "language": language})
    except urllib.error.HTTPError as e:
        return jsonify({"error": f"TTS API error: {e.code}"}), 502
...
def submit_feedback():
    ...
    with get_db_connection() as conn:
        conn.execute(
            """
            INSERT INTO feedback (user_id, history_id, rating, category, comment)
            VALUES (?, ?, ?, ?, ?)
            """,
            (session["user_id"], history_id, rating, category, comment),
        )
        conn.commit()
    return jsonify({"saved": True})
...
def feedback_summary():
    with get_db_connection() as conn:
        ...
        avg = conn.execute(
            "SELECT AVG(rating) as avg FROM feedback WHERE user_id = ?",
            (session["user_id"],),
        ).fetchone()
    return jsonify({
        "average": round(avg["avg"] or 0, 1),
...
def log_attention():
    data = request.get_json(silent=True) or {}
    status = data.get("status", "unknown")   # "focused" | "distracted" | "away"
    history_id = data.get("history_id")
    with get_db_connection() as conn:
        conn.execute(
            "INSERT INTO attention_logs (user_id, history_id, status) VALUES (?, ?, ?)",
            (session["user_id"], history_id, status),
        )
        conn.commit()
